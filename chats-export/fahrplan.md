...
21. **Testprojekt bauen und den mehrstufigen Test fahren.** Fakten: Doku-Kopf (Ende der Entwicklungsphase), 1.5, 2.6, 3.2.3. Ein eigenes claude.ai-Projekt statt des FreeCAD-Projekts, dazu ein Zielrepo. Stufen: Erstlauf, aktives Weiterschreiben eines Chats zwischen zwei Läufen, Sitzungsübergabe, Fortsetzung eines früheren Chats. Beantwortet nebenbei die offene Frage aus 3.1.8. *Warm.*
    ...
    **Der Prüfplan.** Je Schritt, was er beweisen soll und was ein Fehlschlag bedeutet. 21.1 und 21.9 fehlen: der eine schreibt dieses Papier, der andere erzeugt nur Material.

    - **21.2** — *Erwartung:* alle neun Profilmerkmale aus Doku 4.1 sind erzeugt und notiert, samt Anlegedatum des Projekts. *Fehlschlag:* Ein Merkmal lässt sich über die Oberfläche gar nicht erzeugen — dann ist nicht der Test schuld, sondern eine Annahme darüber, wie solche Chats entstehen; Merkmal streichen oder anders herstellen und 4.1 nachziehen.
    - **21.3/21.4** — *Erwartung:* Das Kurzzeitraum-ZIP enthält das neue Projekt mit dem notierten `created_at`, obwohl der gewählte Zeitraum davor liegt, und **keine** seiner Konversationen. *Fehlschlag:* Die Ausnahme der Projektdateien vom Zeitraumfilter ist gekippt — dann entfällt der Sondierungsexport als Verfahren, Doku 1.5 Schritt 0 und 4.2 müssen neu, und der Projektbeginn braucht eine andere Quelle.
    - **21.5** — *Erwartung:* Die Instanz liefert die Rohblöcke in einem Codeblock, und `parse_chat_list` findet jeden Chat mit UUID, `updated_at` und Titel. *Fehlschlag:* `MAPPING_PROMPT` oder das Ausgabeformat von `recent_chats` hat sich geändert — 4.3 nachziehen, Prompt anpassen.
    - **21.6** — *Erwartung:* alle Chats `listed`, die gemeldete Fenstergrenze genau auf dem Projektdatum, keine Tippfehlerwarnung. *Fehlschlag:* `window_start()` rechnet anders als die Tabelle in Vorgabe 2.4 beschreibt — dann ist eine der beiden Seiten falsch, und zwar vor jedem weiteren Schritt zu klären.
    - **21.7** — *Erwartung:* zu jedem Profilmerkmal die zugehörige Datei oder Zählung; `diff` meldet nichts Offenes; `report` und `analyse` stimmen in allen gemeinsamen Zahlen überein. *Fehlschlag:* je nach Merkmal ein Befund in 3.1.1 bis 3.1.4 — das Merkmal sagt, wo.
    ...
    - **21.11** — *Erwartung:* Der zweite Export über genau das errechnete Fenster enthält den fortgeschriebenen Altchat; `convert` ersetzt ihn und **nennt** die entfernten Vorgängerdateien; der Waisen-Scan meldet nichts. *Fehlschlag:* stiller Inhaltsverlust — der gefährlichste Fall überhaupt, weil ein zu knappes Fenster nichts meldet, sondern einfach weniger liefert.
