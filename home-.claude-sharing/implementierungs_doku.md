...
Hat der Nutzer den Abgleich von Hand angehalten (etwa auf Empfehlung der Sitzung, 1.6), gehört auch das in die Meldung — eine vergessene, selbst gesetzte Pause legt den Abgleich sonst unbemerkt still. Der Wächter erkennt sie am `paused`-Feld, das er ohnehin lesen kann; er verändert es nicht (2.1).
...
<!-- Auch im folgenden Absatz prüfen, ob das noch inhaltlich mit dem Code überein stimmt. Prinzip: Der Code gibt vor, was hier steht. -->

**Inhalt und Anzeigedauer** (Entscheidung zu F10). Gemeldet werden die je Richtung übertragenen Bytes als Lebenszeichen und, wenn vorhanden, der **Rückstand** — wie viele Dateien noch auf Übertragung warten. Der Rückstand ist die eigentliche Warnung: „7 Dateien warten" heißt, dass etwas klemmt, und davon erfährt man sonst nie etwas. Die Zahl übertragener Dateien wird bewusst **nicht** gemeldet; sie sagt neben den Bytes nichts Neues.
