...
27. **Ein Skill, der beide Wege anbietet, die Lage feststellt und den passenden vorschlägt.** *Warm für den Lauf, kalt für den Bau.* — **Plan, noch nicht ausgeführt.**
    ...
    **Keine zweite gepflegte Fassung, also keine Drift-Sicherung.** `chat_export_convert.py` liegt genau einmal im Repo, und zwar **im Skill-Ordner** `chat-export/skills/chat-export/` — es ist das eine Skript, das der Skill mitbringt, und der Ordner trägt damit genau die Struktur, die er am Zielort haben wird. Der Nutzer kopiert diesen Ordner als Ganzes, nichts weiter — und er braucht **nichts** darüber hinaus: Sein vollständiger Arbeitsgang, Chats holen und am Ziel ablegen, steckt in diesem einen Skript. Eine Prüfsumme oder ein Versionsabgleich wäre Maschinerie für ein Problem, das diese Anordnung nicht hat.

    **Der Ordner `source/` ist damit entfallen**, denn er hätte nach dem Umzug nichts mehr enthalten, was der Nutzer je kopiert. Die drei übrigen Skripte liegen jetzt dort, wo ihre Aufgabe sie hinstellt: `inspect_export.py` als Wartungswerkzeug dieses Vorhabens (Schemawache gegen Formatänderungen, Kapitel 4) direkt in `chat-export/`; `chat_read_store.py` und `chat_crawl_store.py` in `chat-export/tests/`, weil sie derzeit von nichts als Tests angefasst werden — der Lese-Weg ist nicht lauffähig und dient nur noch als zweite Umsetzung, an der Vorgabe 2.5 gemessen wird, der Crawler wartet auf Punkt 10. Vorgabe 2.9 bleibt unberührt: Sie regelt, **wie** diese Skripte geschrieben sind (eigenständig, Docstring als Betriebsanleitung), nicht wo sie liegen.
