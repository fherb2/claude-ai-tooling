...
Das angestrebte Verhalten ist in `SKILL.de.md`/`SKILL.en.md` und der `README.md`/`README.en.md` des Skills umgesetzt (Anwenderdokumentation, Bedienung und Hintergrund getrennt). Die Mechanik der Browser-Anbindung samt ihrer Fallstricke steht dauerhaft in `chrome-zugriff.de.md`/`chrome-access.en.md` — nicht befristet, weil sie sich bei jeder künftigen Anthropic-Änderung an der Bridge wieder als Nachschlagewerk eignet.
