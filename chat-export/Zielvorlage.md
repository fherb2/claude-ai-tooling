...
1. **Claude in Chrome** ist installiert, **und die Brücke ist einmal eingerichtet.** Die Erweiterung allein genügt nicht: Claude Code redet mit ihr über eine Native-Messaging-Host-Datei, die es beim **erstmaligen Einschalten** der Integration selbst anlegt — im Terminal über `claude --chrome`. Danach muss Chrome einmal vollständig neu starten, weil die Datei nur beim Start gelesen wird. Erkennbar ist der Zustand an ihrem Vorhandensein: `~/.config/google-chrome/NativeMessagingHosts/com.anthropic.claude_code_browser_extension.json`. Sie ist notwendig, aber nicht hinreichend — **getragen wird die Verbindung vom `@browser` je Nachricht** (Abschnitt 2.1). Fehlt es, meldet die Anbindung „Browser extension is not connected", was nach einem kaputten Aufbau klingt und nur heißt, dass diese Nachricht sie nicht angefordert hat. Näheres in `testlauf.md`.
