...
- **`settings-json-snippet.md`** — nur bei Fähigkeiten mit Hook-Auslöser (5.0). Bei einer reinen Hook-Fähigkeit steht sie anstelle der `CLAUDE-snippet.md`; ein Skill kann aber auch **beide** tragen, wenn der stille Trigger die Anlässe abdeckt und der Hook genau den Fall, in dem der Trigger nicht verlässlich zieht (Beleg: `git-branch-model`, 16. September 2026 — Trigger für Release, Zweig, zentrale Datei; Hook für das erste schreibende Git-Kommando, das Sonnet am gemeinsamen Anker mit einem zweiten Skill nicht von selbst zum Laden brachte). Gleicher Aufbau, gleiche Trennlinien-Regel; sie trägt den `settings.json`-Eintrag statt eines CLAUDE.md-Absatzes.
...
**Was eine Kompaktierung für die stillen Trigger dieses Repos bedeutet** (dokumentiert, [Explore the context window](https://code.claude.com/docs/en/context-window), am 2. September 2026 am laufenden System bestätigt, Stelle am 14. September 2026 erneut abgerufen): „The skill listing does not reload" — die Beschreibungsliste der verfügbaren Skills wird nach einer Verdichtung **nicht** wieder eingespielt; die Körper der bereits benutzten Skills dagegen schon („re-injects the skills you invoked").

**Die Skills sind danach nicht verschwunden, sondern nur aus dem Kontext entladen.** Sie kommen mit **denselben** Mitteln zurück wie in einer frischen Sitzung — uneingeschränkt. Die Beschreibungsliste dient laut derselben Quelle dazu, „so Claude knows what it can invoke": Sie ist Voraussetzung für das **Entdecken** eines Skills, nicht für sein Laden. Beleg in der Quelle selbst: Skills mit `disable-model-invocation: true` stehen **nie** in dieser Liste und sind trotzdem jederzeit unter ihrem Namen aufrufbar. Bis zum 14. September 2026 stand hier die gegenteilige Folgerung — ein stiller Trigger zeige nach der Verdichtung „ins Leere". Das war falsch und hat an mehreren Stellen in die Irre geführt.

**Der stille Trigger wird dadurch nicht wertlos, sondern wichtiger.** Die `CLAUDE.md` wird neu geladen, der Trigger überlebt also und nennt den Skill beim Namen; er ist nach einer Verdichtung der einzige verbliebene **automatische** Weg zum Skill. Außer Kraft gesetzt wird umgekehrt die Auslösung über die **Description** — die steht nicht mehr im Kontext. Ein Skill ohne stillen Trigger ist danach nur noch über den Nutzer erreichbar. Damit der Trigger diese Rolle tragen kann, muss er den Skill **beim Namen nennen** und sein Laden anweisen; ein Absatz, der nur auf die Existenz eines Skills hinweist, hilft an dieser Stelle nicht.

**Still in Vergessenheit geraten die Skills auch dann nicht.** `recall-skills-after-compact` legt dem Nutzer unmittelbar nach der Verdichtung vor, welche Skills zuvor geladen waren, sodass er sie sofort von Hand zurückholen kann — noch bevor Trigger oder Description von sich aus erkennen, dass ein Skill wieder gebraucht wird (Praxis des Entwicklers, im Einsatz funktionierend beobachtet).

Für die Messung (Kapitel 4.2) folgt daraus: Eine gemessene Triggerempfindlichkeit gilt dem unkomprimierten Fall. Nach einer Verdichtung trägt allein der `CLAUDE.md`-Trigger weiter; die Description als Auslöser ruht bis zur nächsten Sitzung.

Erster Baustein dieser Art: `recall-skills-after-compact`. Zweiter, in Mischform mit einem stillen Trigger: der Hook von `git-branch-model` (`PreToolUse`), mit zwei Befunden aus der Hooks-Referenz, die beim Bau eines weiteren `PreToolUse`-Hooks vorab bekannt sein müssen: Der `matcher` filtert nur nach Toolname, die Kommandofeinheit liefert das Feld `if` mit Permission-Rule-Syntax (`"Bash(git *)"`); und reiner stdout-Text erreicht die Instanz bei `PreToolUse` **nicht** — anders als bei `SessionStart` —, sondern nur die JSON-Felder `hookSpecificOutput.additionalContext` (Hinweis, hält das Kommando nicht auf) oder `permissionDecision: "deny"` mit `permissionDecisionReason` (Verweigerung, erreicht die Instanz als Werkzeugergebnis). Wer garantieren will, dass etwas **vor** dem Kommando geschieht, braucht die Verweigerung; ein Hook kann sie selbst begrenzen, indem er im Transkript (`transcript_path`) nachsieht, ob die Bedingung inzwischen erfüllt ist — so blockiert `notify_branch_model.py` höchstens einmal je Sitzung.
...
| `implementation-doc.md`, `skill-dev-doc.md` | `chat-export`, `web-code-editing` | Dokumente des Repositories, die nie mitreisen |
