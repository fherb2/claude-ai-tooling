...
3. **`settings-json-snippet.de/en.md` für `git-branch-model`** anlegen, gleich gebaut wie bei `recall-skills-after-compact`: Datumszeile, kursive Kopfnotiz (JSON wird eingefügt, nicht angehängt; Pfad trägt `$HOME`, kein Platzhalter — Kap. 5.0 nennt den Schaden: ein `/home/<user>/…`-Platzhalter wird mitkopiert, übersehen, und der Hook scheitert still), Trennlinie, der zu übernehmende `hooks`-Block. Die **README** bekommt im Installationskapitel einen Schritt „Hook verdrahten" nach Vorlage C aus Kap. 6.1 — **zusätzlich** zum Trigger-Schritt, nicht an seiner Stelle, denn der Trigger deckt weiter die Anlässe ab (Release, Zweig, zentrale Datei) und der Hook nur den Commit-Fall. Die `SKILL.md` bleibt unverändert; sie ist kein reiner Slash-Zweitzugang wie bei `recall-skills-after-compact`, sondern der eigentliche Skill. (Der Plan hatte hier zuerst die `SKILL.md` genannt — falsch zugeordnet, am 16. September 2026 berichtigt.) Zwei Recherchebefunde aus der Hooks-Referenz, die den Bau bestimmen: `PreToolUse`-Matcher filtern nur nach Toolname, die Kommandofeinheit liefert das Feld `if` mit Permission-Rule-Syntax (`"Bash(git *)"`); und reiner stdout-Text landet bei `PreToolUse` **nicht** im Kontext — nur das JSON-Feld `hookSpecificOutput.additionalContext` erreicht die Instanz, anders als bei `SessionStart`.
...
**Stand 16. September 2026:** Punkte 1–5 und 7 sind ausgeführt. Der Entwickler hat sich nach der ersten Messreihe gegen die „leichte Variante" und für die **Einmal-Blockade** entschieden: Der Hook verweigert das erste schreibende Git-Kommando der Sitzung mit Begründung, solange das Transkript keinen Aufruf von `git-branch-model` zeigt — damit läuft der Abgleich *vor* dem ersten Schreiben; ein bloßer `additionalContext` hält das Kommando nicht auf. Messung der Blockade: 3 von 3 mit Datei abgewiesen, Skill geladen, Kommando wiederholt; 0 von 3 Eingriffe ohne Datei. Offen bleiben **Punkt 6** (Eintrag in `~/.claude/settings.json` und in die `settings.json` dieses Repos auf `repo-management`, jeweils mit Freigabe) und die Wirkungsprüfung in einer echten Sitzung. Bekannte Grenze des Messaufbaus: Im `-p`-Lauf verweigert die Berechtigungsprüfung des Nutzers `git add`/`commit` ohnehin, weshalb das Kommando nach der Blockade im Test nie wirklich durchlief — die Reihenfolge Blockade → Skill → Wiederholung ist belegt, der erfolgreiche Commit danach erst in der echten Sitzung zu sehen.
