...
- **Skripte:** Bezeichner, Kommentare, Docstrings, Hilfetexte und Ausgaben an den Nutzer. Dazu zwei aus der Dateinamen-Klasse hierher verschobene, weil code-gekoppelte Namen: `konfliktloesung.md` (home-sharing, Produktdatei des laufenden Dienstes samt Unit und Installskripten) und die Testdateien `test_wegegleichheit.py`/`wegegleichheit_referenz.py`. (`protokoll.json` → `protocol.json` ist am 26. August 2026 erledigt — Entscheidung des Entwicklers: kein Lesepfad für den alten Namen, Altbestände auf allen Systemen mit umbenannt; vermerkt in Vorgabe 2.4 von chat-export.)
