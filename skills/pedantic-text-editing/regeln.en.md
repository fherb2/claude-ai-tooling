...
**Deferred findings from excluded classes** may be noted in the head — but only what caught the eye during the pass anyway: no separate search pass, **no target count**, zero is a normal result. Note them completely or not at all — a shortened list would look like an inventory it is not. (Four out of four rounds without an explicit instruction delivered exactly two deferred findings, the first with no template before it — the model's own production, not a carry-over; seen on the live system, 25 August 2026.)
