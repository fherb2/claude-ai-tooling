...
Dagegen setzt der Skill drei Dinge. Erstens die **Verifikationspflicht**: Was nur in einer Suchzusammenfassung steht, gilt als unbestätigt, bis die tragende Seite abgerufen und die Aussage dort gesehen wurde; jeder Fund wird als *belegt*, *unbestätigt* oder *eigenes Modellwissen* gekennzeichnet. Ist die Primärquelle ein PDF, das nicht vollständig in den Kontext passt, gilt das über Blättern hinaus: Claude sucht den Volltext gezielt durch und hält nötigenfalls an, um ein fehlendes Extraktionswerkzeug nachinstallieren zu lassen. Zweitens **sechs Suchoperatoren** als Pflichtrepertoire — umformulieren (auch in andere Sprachen), auf eine ergiebige Website eingrenzen, die Ebene wechseln (nicht das Dokument suchen, sondern das Register, das es listet), Entitäten wie Autoren und Zeitschriften verfolgen, den Kanal wechseln, verifizieren. Drittens den **Frontier-Bericht**: Bleibt die Suche erfolglos, ist „nichts gefunden" als Antwort unzulässig; stattdessen listet Claude, was versucht wurde und welche Suchwege noch offen sind — die Entscheidung über den Abbruch liegt damit beim Nutzer, nicht bei Claudes Ermüdung.
...
**Warum die PDF-Prüfung auf claude.ai praktisch nie zuschlägt.** Passt ein PDF nicht in den Kontext, braucht die Verifikation ein Extraktionswerkzeug. In der Code-Execution-Sandbox von claude.ai sind PDF-Bibliotheken fest vorinstalliert, und die Sandbox hat aus Sicherheitsgründen ohnehin keinen Internetzugang — sie muss also nichts nachladen (Stand 2026-09-25, geprüft gegen platform.claude.com/docs, Abschnitt „Code Execution Tool"; kann sich mit künftigen Änderungen der Sandbox überholen). In Umgebungen ohne diese Sandbox, etwa Claude Code auf der eigenen Maschine, ist das nicht garantiert — dafür ist die Prüfung im Skill da.
