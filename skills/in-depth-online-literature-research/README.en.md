...
*[Deutsche Fassung](https://github.com/fherb2/claude-ai-tooling/blob/master/skills/in-depth-online-literature-research/README.md)*
...
Against this the skill sets three things. First, the **duty to verify**: whatever stands only in a search summary counts as unconfirmed until the page carrying it has been fetched and the statement seen there; every finding is marked as *verified*, *unconfirmed* or *own model knowledge*. If the primary source is a PDF that does not fit into context in full, this goes beyond paging through: Claude searches the full text in a targeted way and pauses if needed to have a missing extraction tool installed. Second, **six search operators** as a mandatory repertoire — reformulate (including into other languages), narrow down onto a productive website, change level (do not search for the document but for the register that lists it), follow entities such as authors and journals, change channel, verify. Third, the **frontier report**: if the search stays unsuccessful, "nothing found" is not an admissible answer; instead Claude lists what was tried and which search paths are still open — which puts the decision to stop with the user, not with Claude's fatigue.
...
**Why the PDF check practically never fires on claude.ai.** If a PDF does not fit into context, verification needs an extraction tool. The code-execution sandbox on claude.ai comes with PDF libraries preinstalled, and the sandbox has no internet access anyway, for security reasons — so it has nothing to fetch (as of 2026-09-25, checked against platform.claude.com/docs, "Code Execution Tool" section; may be overtaken by future sandbox changes). In environments without this sandbox, such as Claude Code on your own machine, that is not guaranteed — which is what this check in the skill is for.
