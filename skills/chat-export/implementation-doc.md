...
**Status: Am echten Lauf erprobt** — über drei unabhängige Sitzungen mit kaum Zutun des Nutzers, zuletzt am Realdaten-Großlauf (3.1.7). `README.md` (deutsch) und `README.en.md` sind geschrieben, reine Anwenderdokumentation ohne Statushinweis und ohne Entwicklungsangaben — bewusst abweichend von `skill-dev-doc.md` 6.1. Der Grund: Was ein Nutzer kopiert, soll ihm die Bedienung sagen und nicht den Entwicklungsstand; der gehört in die Bereichs-README, wo er auch gepflegt wird.

Der Skill ist die Klammer um das Skript: Er führt den Nutzer durch beide Wege, ohne ihm die Entscheidung abzunehmen. Sein Ordner ist `skills/chat-export/`. Ins Installationspaket gehen daraus: `chat_export_convert.py`; `inspect_export.py` als Diagnosewerkzeug des Anwenders (3.2); die Anwenderdokumentation in zwei Sprachfassungen (`README.md` deutsch, `README.en.md` englisch); die Anweisungsdatei ebenso zweisprachig (`SKILL.de.md`/`SKILL.en.md`, beim Packen zu `SKILL.md` umbenannt — Konvention aus `skill-dev-doc.md` 5.1); und die Fehlerdiagnose `bridge-diagnosis.de.md`/`bridge-diagnosis.en.md`. **Mehr bekommt ein Nutzer nicht**: Implementierungsdoku, Fahrplan, Testsuite und der Testweg zur Chrome-Anbindung bleiben im Repository und kommen in der Anweisungsdatei nicht vor. Welche Dateien ein Paket tatsächlich trägt, entscheidet die Aufzählung beim Bauen (`skill-dev-doc.md` 5.3).
...
**Voraussetzungen, die der Nutzer herstellen muss** und die der Skill nicht umgehen kann: angehängte Browser-Werkzeuge (in der VS-Code-Erweiterung `@browser` je Nachricht), ein laufender und bei claude.ai angemeldeter Chrome mit eingeschalteter Erweiterung, und in Chrome ausgeschaltetes Nachfragen nach dem Speicherort — ein Dateidialog blockiert die Anbindung vollständig. Die Kontobindung ist dabei **keine** Bedingung: Chrome und Claude Code dürfen an verschiedenen Konten hängen (belegt, `chrome-access.de.md`/`chrome-access.en.md`). **Anthropics eigene Fehlermeldung behauptet das Gegenteil** — sie verlangt, „logged into claude.ai with the same account as Claude Code“ zu sein. Eine Fehlermeldung zählt mögliche Ursachen auf und ist damit keine dokumentierte Bedingung; sie steht in der Beleglage (2.1) unter der zweimal reproduzierten Beobachtung, nicht über ihr. Wer das nicht weiß, diagnostiziert einen Bridge-Fehler falsch und schickt den Nutzer zum Angleichen der Konten — in der Anwenderdokumentation dieses Skills ist genau das vom 22. bis 31. August 2026 so gestanden. Die Abgrenzung der Fehlerbilder steht deshalb als `bridge-diagnosis.de.md`/`bridge-diagnosis.en.md` im Skill und wird von der `SKILL.md` erst geladen, wenn die Anbindung wirklich klemmt. Was der Skill sieht, ist immer die Sitzung, die im Tab gerade aktiv ist — deshalb **nennt** er das erkannte Konto, statt eines vorauszusetzen.
...
- **warm** — nur mit Zugriff auf ein echtes Konto: ein angemeldeter Browser für die Web-Endpunkte (der Regelfall, Voraussetzungen in `chrome-access.de.md`), ein claude.ai-Projekt für `recent_chats`, Upload und Projektwissen, oder ein Claude-Code-Projekt als Zielort. Braucht Vorbereitung, ist nicht beliebig wiederholbar und hinterlässt Spuren an der Quelle.
