...
**Befund B-06 (2026-09-25): Diese Vorgabe und der ausgearbeitete Ausgabevertrag zählen verschieden.** Oben stehen vier Aussageklassen, und die Befundklasse heißt `FINDINGS` im Plural. Der Ausgabevertrag in Kapitel 3.6.3 kennt sechs: `OK`, `ITEM` für einen Gegenstand einer Liste, `FINDING` im Singular für einen einzelnen Befund, `DECIDE`, `FAILED` und `SUMMARY` als letzte Zeile jeder Ausgabe. Das ist kein Detailunterschied, denn die Klasse steht am Anfang jeder Zeile und wird vom Skript wörtlich ausgegeben; eine Instanz, die `FINDINGS` erwartet und `FINDING` liest, findet nichts. Welche Seite recht hat, ist offen: Entweder ist die Vorgabe die ältere und knappere Fassung, die nachzuziehen ist — dann sind es sechs Klassen, und `ITEM` und `SUMMARY` sind Zeilenarten, die sie bisher nicht kannte. Oder die Vorgabe ist bewusst gröber und nennt nur die vier Ausgänge eines Laufs, während 3.6.3 die Zeilenarten darunter beschreibt; dann muss wenigstens der Plural verschwinden. Zu entscheiden: welche der beiden Lesarten gilt.
...
**Befund B-04 (2026-09-25): Die Blockade ist inzwischen aufhebbar, hier steht es nicht.** Am 2026-09-24 wurde entschieden, dass der Entwickler die Commit-Blockade für einen einzelnen Commit ausdrücklich aufheben kann, statt den Hook im Projekt insgesamt abzuschalten — nachzulesen in Kapitel 1.3.5, wo auch der Grund steht: Ein Mechanismus, den man nur ganz oder gar nicht haben kann, wird beim ersten unpassenden Moment ganz abgeschaltet und kommt nicht zurück. Der Satz oben und der Ablaufschritt 6 in Kapitel 1.8 beschreiben die Blockade noch ohne diese Möglichkeit. Beide lesen sich damit strenger, als die Sache ist. Zu entscheiden: ob die Aufhebbarkeit in diese Vorgabe gehört — sie ist prüfbar, also spricht einiges dafür — oder ob sie eine Einzelheit der Hooks bleibt und hier nur ein Verweis steht.

**Befund B-08 (2026-09-25): Der Fingerabdruck lässt einen Hook doch schreiben.** Der erste Satz oben ist ausnahmslos. Am 2026-09-24 wurde für den Fingerabdruck — den Kurzhash des Definitionssatzes, an dem sich erkennen lässt, dass eine Festlegung umformuliert wurde — ergänzt: Weicht er ab, vergleicht das Skript erst mechanisch, wie viele Zeichen sich unterscheiden; liegt es unter einer Schwelle, gilt das als Formatierung oder Rechtschreibung, es gibt keine Meldung, und das Skript berechnet den Wert **still neu** (Kapitel 1.7.6 und 3.6.7). Diese Neuberechnung schreibt ins Register. Das Kommando `check`, das sie auslöst, läuft aber genau in zwei Hooks — vor dem Commit und am Sitzungsstart. Damit ändert ein Hook eine Datei. Zu entscheiden: ob die stille Neuberechnung nur bei ausdrücklichem Aufruf geschieht und der Hook bloß nicht meldet — dann bleibt die Vorgabe ausnahmslos —, oder ob sie überall geschieht und die Vorgabe eine benannte Ausnahme für den eigenen Registerwert trägt.
...
**Befund B-05 (2026-09-25): Ein Parameter hat keinen Standardwert, und zwei weitere haben keinen im Sinne dieses Satzes.** Am 2026-09-24 wurde für `mode` — den Schalter, ob der Skill in diesem Projekt überhaupt geführt wird — entschieden, dass er bei fehlender Datei nicht stillschweigend als eingeschaltet gilt, sondern einmal je Sitzung erfragt wird (Kapitel 1.7.7). Damit hat er keinen Standard**wert**, sondern ein Standard**verhalten**, und der erste Satz oben stimmt nicht mehr. Dasselbe in schwächerer Form bei `doc_dir` und `planned_steps`: Ihr Eintrag in der Tabelle lautet „abgelesen" — auch das ist kein Wert, sondern eine Anweisung, ihn zu beschaffen. Zu entscheiden: ob der Satz auf „Standardwert oder Standardverhalten" erweitert wird, oder ob die drei Fälle sauberer zu trennen sind — ein Wert, den man setzen kann, ein Ergebnis, das abgelesen und gespeichert wird, und eine Frage, die gestellt wird. Die zweite Lesart wäre zugleich die Antwort auf den Aufnahmetest weiter unten, der heute nur Werte kennt, die man setzt.
