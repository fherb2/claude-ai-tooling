...
**Sechs Kapitel werden wörtlich zum Text des Skills: 3.1 als `rules-hardness.md`, 3.2 als `rules-register.md`, 3.4 als `rules-planning.md`, 3.5 als `rules-startup.md`, 3.10 als Körper der `SKILL.md`, 3.11 als der Trigger in `CLAUDE.md`.** Sie sind dann nicht eine zweite Beschreibung dessen, was Kapitel 1 beschreibt, sondern das Erzeugnis selbst — Kapitel 1 beschreibt, Kapitel 3 **ist**. Wo ein solcher Text eine Absicht wiederholen muss, damit er für sich lesbar bleibt, geschieht das knapp und mit Verweis auf die Stelle in Kapitel 1, die sie normativ trägt.

**Damit das schon jetzt sichtbar ist, während diese sechs Kapitel noch entstehen, tragen sie eine eigene Schreibweise** (entschieden am 2026-09-24). Ein Zitat (`> …`) markiert den ausformulierten Zieltext — das, was unverändert in den Regelteil wandert. Ausgenommen ist ein Zitat, das mit `**[Q-` beginnt: Das bleibt ein Entscheidungsblock, kein Zieltext, erkennbar am ersten Zeichen; solche Blöcke verschwinden ohnehin, sobald sie aufgelöst sind. Alles außerhalb der Zitate ist projektbegleitender Text dieser Entwicklung — Begründung, Verweis, Kontext —, auch wenn er die drei Prüfungen oben noch nicht besteht; er schrumpft mit fortschreitender Ausarbeitung, bis im Grenzfall nur noch ein Verweis übrigbleibt. Die Kapitel- und Abschnittsnummerierung in diesen sechs Kapiteln gehört dieser Entwicklung, nicht dem Zieltext — sie muss im Regelteil nicht wiederkehren, und Verweise darauf (etwa „Kapitel 1.3.2") sind vor der Übernahme in den Skill durch die dann gültige Zielstruktur zu ersetzen. Für die übrigen Kapitel dieses Segments (3.3, 3.6 bis 3.9) gilt uneingeschränkt weiter, was oben steht: Sie spezifizieren Code, Konfiguration oder einen Vorgang, keinen Text, den eine Instanz zur Laufzeit liest.

**Befund B-13 (2026-09-25): Die Konvention hat keine Form für Beispiele im Zieltext.** Beim Übernehmen in den Skill fällt genau eine Zitatebene weg — aus `> Text` wird `Text`. Das trägt, solange der Zieltext durchgehend eine Ebene tief ist. Es bricht, sobald darin selbst ein Zitat vorkommt, und das kommt vor: Kapitel 3.2.2 zeigt einen Beispielabsatz mit drei Markern, damit der Leser die Schreibweise an echtem Text sieht. Dieses Beispiel steht heute auf derselben Ebene wie der Zieltext, der es einleitet. Nach dem Abziehen einer Ebene wäre es kein Beispiel mehr, sondern liefe als normaler Absatz in den Regeltext hinein — und der Satz davor, der es ankündigt, zeigte ins Leere. Der naheliegende Weg ist ein verschachteltes Zitat (`>>`) für alles, was im Zieltext selbst ein Zitat sein soll; dann bleibt nach dem Abziehen ein Zitat übrig. Zu entscheiden: ob das so festgelegt wird, und ob Beispiele im Zieltext überhaupt bleiben sollen oder erst beim Zusammensetzen entstehen.

## 3.1 Härte einer Festlegung — Ableitung und Wirkung

Finaler Stand von Arbeitspunkt 1 (2026-09-17), mit zwei nachträglichen Anpassungen aus Arbeitspunkt 2: Die Ereigniszeile „bestätigt" des Vorläufers (Anhang A) heißt `upheld` statt `confirmed`, weil `confirmed` zugleich ein Statuswert ist (Kapitel 3.2) — vom Entwickler am 2026-09-24 bestätigt (vormals Q-02); und „Segment 2" ist ersetzt durch „Abschnitte mit der Funktion `global`" (Kapitel 1.3.3). Adressat des folgenden Zitats ist die Instanz („Du"), der Mensch heißt „der Entwickler".
...
Die Begründung, warum vier naheliegende Eingänge — Alter der Entscheidung, Umbaukosten, Gewichtung, neues Wissen — fehlen, steht in Kapitel 1.3.1 und gehört nicht hierher. Was für die Instanz operativ bleibt:
