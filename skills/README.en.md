...
| [`🚧_web-code-editing/`](🚧_web-code-editing/README.md) (in German) | **Handling code artifacts in the web frontend**: when code becomes an artifact and when a change instruction in the chat, and in what form changes to already adopted code are communicated.                                                        |
...
2. **Copy the folder — all files.** It already has exactly the structure it will have at the target location and keeps its name. Files of another language need not come along; everything else does, because a skill may hold more than `SKILL`, `README` and `CLAUDE-snippet` — a lazily loaded rules part, say, or a script it does not work without. The date lines of the copied files later show which state the installation is from. **Then rename the SKILL file of the chosen language to `SKILL.md`:** Claude Code presupposes exactly this name, a `SKILL.de.md` on its own is not a skill. Instead of renaming it may also be placed additionally under this name; likewise a `README.en.md` may become `README.md` in an English installation. All other files keep their names — the `SKILL.md` points to them, and a renamed rules part is no longer found.
