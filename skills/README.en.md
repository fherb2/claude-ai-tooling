...
| [`git-branch-model/`](git-branch-model/README.en.md)<br>✅☑ | **Running a Git project with a development, a release and a management branch** — topic branches by merge, release transfer by merge or file by file, and an orphan management branch for files that must be identical on every branch: distributed by overlaying instead of merging, with a sync that overwrites the outdated and reports the new. |
| [`git-workbench/`](git-workbench/README.en.md)<br>✅☑ | **How a Claude session commits** — directly on the current branch, on a workbench with safety commits and a squash at the end, or isolated in its own Git worktree for several sessions at once; plus a push rule against branches stranded when switching machines. |
...
| [`🚧_software-design-doc/`](🚧_software-design-doc/doc/1_overview.md) (in German) | **A documentation standard for planning before coding and for the running record of what was implemented** — what is being built, which decisions were taken and why this way and not another. Documentation accompanying a software project, as the basis for coding, for debugging and for the later closing and user documentation. **Being rebuilt:** hardness rules, a register with marks, and document roles; the link leads to the implementation documentation of the rebuild, where the predecessor is archived in Appendix A. |
