...
**An idea, not on the agenda yet — a working copy in the working directory:** copy the files worked on during a session out of project knowledge into Claude's own working directory once, and carry them along there for the session, instead of hauling every version through the context again. The hope: Claude finds its way around coherent code better than around fragments scattered across the chat, and more substantial trial runs during planning become possible.

Whether that holds is open. Before going at it, the actual conditions have to be checked first — does the working directory stay stable across a whole session, how does it relate to `/mnt/project/` and `/mnt/user-data/outputs` — and the reservation has to be settled that a working copy is a second state of the same code, and thus sharpens exactly the drift that "Check whether your state is current" warns about.
