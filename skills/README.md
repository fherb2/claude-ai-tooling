...
2. **Eine vollständige Sprachversion des Ordners kopieren.** Der Ordner hier hat bereits genau die Struktur des Zielorts und behält seinen Namen. Mit gehören alle Dateien der gewählten Sprache — README und `CLAUDE-snippet` eingeschlossen; ihre Datumszeilen zeigen später, von welchem Stand die Installation ist. **Zwingend ist allein der Name `SKILL.md`:** Claude Code erkennt keinen anderen, eine `SKILL.de.md` für sich ist kein Skill. Ob die gewählte Fassung dafür umbenannt oder zusätzlich unter diesem Namen abgelegt wird, ist gleichgültig; ebenso darf eine `README.en.md` bei englischer Installation zu `README.md` werden.
