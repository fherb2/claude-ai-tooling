...
1. **The project has agreed on the worktree working model** — recognizable by the file `.claude/git-worktree-model.json`. Then the everyday procedure below applies; the names of branches, storage location and infra files are in that file, not in this skill.
2. **No model agreed, but a second session is working or has been announced.** Then the immediate rule (next section) applies, and the user is offered the initial setup of the model — briefly and without pushing, since it changes their way of working.
3. **Neither** — a single session, no model: this skill then demands nothing.
